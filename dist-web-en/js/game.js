...
const game = {
  ...
  hoverWerte(b) {
    ...
    const rows = [['Structure', Math.ceil(b.hp) + '/' + b.maxHp]];
    if (d.turret) {
      const legt = !!d.minen;                    // der Minenleger zielt nicht
      rows.push([legt ? 'Damage per mine' : 'Damage', ab(this.stat(b, 'damage'))]);
      rows.push([legt ? 'Laying radius' : 'Range', ab(this.stat(b, 'range')) + ' tiles']);
      rows.push([legt ? 'Mine every' : 'Shot every', dez(this.cooldownOf(b), 2) + ' s']);
      rows.push(['Steady draw', ab(this.drawOf(b)) + '/s']);
    }
    if (d.repair) {
      rows.push(['Repair', ab(this.repairRate(b)) + '/s']);
      rows.push(['Range', ab(this.stat(b, 'range')) + ' tiles']);
      rows.push(['Steady draw', ab(this.drawOf(b)) + '/s']);
    }
    if (d.absorb) {
      rows.push(['Absorbs', Math.round(this.absorbOf(b) * 100) + '%']);
      rows.push(['Reserve', Math.round(b.puffer || 0) + '/' + this.schildPool(b)]);
      rows.push(['Range', ab(this.stat(b, 'range')) + ' tiles']);
    }
    if (d.regen) rows.push(['Yield', '+' + d.regen * b.level + '/s']);
    if (d.capacity) {
      rows.push(['Storage', '+' + Math.round(this.capOf(b))]);
      rows.push(['Carries', '+' + akkuFlow(b) + '/s']);
    }
    if (d.supply) {
      rows.push(['Grid radius', ab(this.netzRadius(d)) + ' tiles']);
      if (b.supplied && b.node)
        rows.push(['Line load', ab(Math.max(0, b.node.through)) +
                                   ' / ' + Math.round(b.node.cap) + '/s']);
    }
    // Eine Drosselung sieht man dem Bau sonst nicht an
    if (d.turret && b.supplied && b.flow < 0.995)
      rows.push(['Grid throttle', '−' + Math.round((1 - b.flow) * 100) + '%']);
    return rows;
  },
  ...
  repair(b) {
    if (b.hp >= b.maxHp) return toast('Undamaged');
    const c = this.repairCost(b);
    if (this.matter < c) { SFX.deny(); return toast('Not enough matter'); }
    this.matter -= c;
    b.hp = b.maxHp;
  ...
  repairAll() {
    ...
    if (!kaputt.length) return toast('Nothing is damaged');
    let n = 0, summe = 0;
    ...
    if (!n) { SFX.deny(); return toast('Not enough matter'); }
    SFX.repair(0);
    toast(n + (n === 1 ? ' building' : ' buildings') + ' repaired  −' + summe + ' matter');
  },
  structureOf(b) {
  ...
  build(type, x, y) {
    ...
    if (!this.free(x, y)) {
      SFX.deny();
      return toast(this.boden(x, y) === BODEN.truemmer ? 'Rubble — nothing can go here'
                                                       : 'Tile occupied');
    }
    if (this.matter < cost) { SFX.deny(); return toast('Not enough matter'); }
    this.matter -= cost;
  ...
  verschiebeStart(b) {
    ...
    if (this.matter < c) { SFX.deny(); toast('Not enough matter'); return false; }
    this.tool = null;
    this.bauplan = null;
    this.verschieben = b;
    this.select(b);
    toast('Pick a new tile for ' + b.def.name + '  −' + c);
    return true;
  },
  verschiebeAbbrechen() {
  ...
  verschiebeZu(x, y) {
    ...
    if (!this.free(x, y)) {
      SFX.deny();
      toast(this.boden(x, y) === BODEN.truemmer ? 'Rubble — nothing can go here' : 'Tile occupied');
      return false;
    }
    const c = this.moveCost(b);
    if (this.matter < c) { SFX.deny(); toast('Not enough matter'); return false; }
  ...
  bauplanStart() {
    ...
    if (!this.buildings.size) { SFX.deny(); toast('Nothing built yet'); return false; }
    this.tool = null;
    this.verschieben = null;
    this.bauplan = { achse: 'x', ziel: 1 };
    if (this.hover.inside) this.bauplanRichtung(this.hover.x, this.hover.y);
    this.select(null);
    toast('Which side? Move the pointer, click to place, Esc cancels');
    return true;
  },
  bauplanAbbrechen() {
  ...
  bauplanBauen() {
    ...
    this.bauplan = null;
    if (!n) { SFX.deny(); toast('Nothing to mirror here'); return 0; }
    for (const z of ziele) {
    ...
    this.recomputeSupply();
    SFX.build();
    toast('Blueprint mirrored: ' + n + (n === 1 ? ' building' : ' buildings') + '  −' + summe);
    this.merken();
    return n;
  },

  upgrade(b) {
    if (b.level >= UPGRADE.maxLevel) { SFX.deny(); return toast('Max level'); }
  ...
  hurt(e, dmg, kind, quelle) {
    ...
    if (e.hp <= 0) {
      ...
      if (e.def.boss) {                           // Bosse zahlen sich aus
        ...
        SFX.bossDown();
        toast(e.def.name.toUpperCase() + ' DEFEATED  +150 matter');
      }
    }
  },
  ...
  damageCore(dmg, enemy) {
    ...
    if (this.coreHp <= 0 && !this.over) {
      ...
      showOverlay('CORE LOST',
        'Waves survived: ' + erg.eintrag.wave + '.' +
        (erg.platz === 0 ? ' That is your best run.' : '') +
        (this.tagesfeld() ? ' Daily map of ' + datumKurz(this.tagesTag) +
                            ' — everyone plays this map today.' : ''),
        bestenlisteHtml(erg.liste, erg.eintrag));
      nameFeldVerdrahten(erg);
      const zweit = el('ovBtn2');
      zweit.hidden = false;
      zweit.textContent = 'Copy result';
  ...
  wavePreview() {
    ...
    return 'Wave ' + (this.wave + 1) + ' from ' + dirs.join(', ') + ':  ' + parts.join('   ·   ');
  },
  ...
  startWave() {
    ...
    SFX.waveStart();
    if (sturm) SFX.storm();
    toast('WAVE ' + this.wave + (sturm ? ' — ' + sturm.name.toUpperCase() : ''));
  },
  ...
  spawn(s) {
    ...
    if (s.boss) {
      ...
      SFX.bossSpawn();
      toast(e.def.name.toUpperCase() + ' APPEARS');
    }
    return e;
  },
  ...
  update(dt) {
    ...
    if (this.modeTimer > 0) {                 // Anlauf des Kernmodus
      ...
      if (this.modeTimer === 0) {
        ...
        this.recomputeSupply();
        SFX.modeReady();
        toast('Core mode: ' + CORE_MODES[this.coreMode].name);
      }
    }
    ...
    if (this.phase === 'build') {
      ...
      if (!this.spawnQueue.length && !this.enemies.length) {
        ...
        SFX.waveClear();
        toast('Wave ' + this.wave + ' repelled  +' + praemie + ' matter');
        this.planNext();
        this.openDraft();
        this.merken();
      }
    ...
    if (this.alarm.on) {
      ...
      if (!was && !this.alarm.seen) {
        ...
        toast('Core exposed — no tower covers this side');
      }
    }
    ...
    for (const b of this.buildings.values()) {
      ...
      if (b.type === 'akku' && b.supplied && b.reserve && this.phase === 'combat' &&
        ...
        SFX.reserve();
        toast('Peak Load — ' + Math.round(menge) + ' energy fed in');
      }
    }
  ...
  drohneTickt(b, dt) {
    ...
    if (b.level >= UPGRADE.maxLevel && b.reserve && this.phase === 'combat') {
      ...
      if (not) {
        ...
        toast('Emergency Weld — ' + not.def.name + ' fully restored');
      }
    }
  ...
  setMode(i) {
    ...
    if (!m) return;
    if (this.modeTimer > 0) { SFX.deny(); return toast('Core is still switching'); }
    if (i === this.coreMode) return;
    this.coreModeNext = i;
    this.modeTimer = this.modeSwitchTime();
    this.recomputeSupply();                   // Anlauf drosselt sofort
    SFX.modeSwitch();
    toast(m.name + ' — ' + dez(this.modeTimer) + ' s to switch');
    updateInspector();
  },
  ...
  usePower(id) {
    ...
    if (this.phase !== 'combat') { SFX.deny(); return toast('Core commands only work during combat'); }
    if (this.cooldowns[id] > 0) {
      SFX.deny();
      return toast(p.name + ' — ready in ' + Math.ceil(this.cooldowns[id]) + ' s');
    }
    const kosten = this.powerCost(p);
    if (this.energy < kosten) { SFX.deny(); return toast('Buffer too low for ' + p.name); }
    if (id === 'pulse' && ![...this.buildings.values()].some(b => b.hp < b.maxHp)) {
      SFX.deny(); return toast('Nothing is damaged');
    }
    this.energy -= kosten;
    ...
    if (id === 'discharge') {
      ...
      SFX.discharge();
      toast('Discharge — ' + Math.round(dmg) + ' damage all around');
    } else if (id === 'surge') {
      this.surge = p.time;
      SFX.surge();
      toast('Grid Surge — ' + p.time + ' s of double damage');
    } else if (id === 'pulse') {
      let n = 0;
      for (const b of this.buildings.values()) {
      ...
      SFX.pulse();
      toast('Repair Pulse — ' + n + (n === 1 ? ' building' : ' buildings') + ' repaired');
    }
    updateInspector();
  },
  ...
  cycleTarget(b) {
    ...
    SFX.select();
    toast(b.def.name + ' targets: ' + TARGETS[b.ziel].name);
    updateInspector();
    this.merken();
  },
...
function drawBuilding(b) {
  ...
  if (b.def.turret && b.prio !== 1) {            // abweichende Lastpriorität
    ...
    ctx.font = '600 ' + 9 * UIK + 'px sans-serif';
...
function drawBauplanGhost() {
  ...
  const ty = p.achse === 'y' ? CORE_PX.y + p.ziel * H * .32 : 150 * UIK;
  ctx.textAlign = 'center';
  ctx.font = '' + 13 * UIK + 'px sans-serif';
  ctx.fillStyle = n ? '#ffd166' : '#ff5d73';
  ctx.fillText(n ? 'Mirror blueprint: ' + n + (n === 1 ? ' building' : ' buildings') + ' for ' + summe + ' matter'
                 : 'Nothing to mirror here', tx, ty);
  ctx.font = '' + 11 * UIK + 'px sans-serif';
  ctx.fillStyle = '#9fb3c8';
  ctx.fillText(ziele.length > n ? (ziele.length - n) + ' more are too expensive  ·  Click mirrors, Esc cancels'
                                : 'Click mirrors, Esc cancels', tx, ty + 17 * UIK);
  ctx.textAlign = 'left';
  ctx.restore();
}
...
function drawHoverKarte() {
  ...
  const titel = b.def.name + '  Level ' + b.level;

  const PAD = 9 * UIK, ZEILE = 15 * UIK, KOPF = 19 * UIK, LUECKE = 16 * UIK;
  ctx.font = '' + 12 * UIK + 'px sans-serif';
  ...
  if (kalt) breite = Math.max(breite, ctx.measureText('no grid — not firing').width);
  ...
  const sperre = ins.hidden ? null
    : { x: W - (12 + ins.offsetWidth) * UIK, o: 64 * UIK, u: (64 + ins.offsetHeight) * UIK };
  const y = clamp(game.hover.y * GRID.cell - 8, 62 * UIK, H - h - 84 * UIK);  // zwischen Kopf- und Taskleiste
  ...
  ctx.fillText(titel, x + PAD, y + PAD + 11 * UIK);
  ctx.font = '' + 11 * UIK + 'px sans-serif';
  let zy = y + PAD + KOPF + 8 * UIK;
  for (const [k, v] of rows) {
    ctx.fillStyle = '#9fb3c8';
  ...
  if (kalt) {
    ...
    ctx.fillText('no grid — not firing', x + PAD, zy);
  }
  ctx.restore();
}
...
function drawGhost() {
  ...
  if (!powered && def.needsPower) {
    ctx.fillStyle = '#ffa14a'; ctx.font = '' + 11 * UIK + 'px sans-serif'; ctx.textAlign = 'center';
    ctx.fillText('no grid', px, py - GRID.cell * .8);
    ctx.textAlign = 'left';
  }
}
...
function buildShop() {
  ...
  for (const [type, def] of Object.entries(BUILDINGS)) {
    ...
    d.className = 'card';
    d.dataset.type = type;
    d.title = def.name + ' — ' + def.desc + '  (key ' + def.key.toUpperCase() + ')';
...
function buildPowers() {
  ...
  for (const p of POWER_LIST) {
    ...
    d.className = 'power';
    d.dataset.id = p.id;
    d.title = 'Core command ' + p.name + ' — ' + p.desc +
              '  (key ' + p.key.toUpperCase() + ', the number is its energy cost from the buffer)';
...
function buildModes() {
  ...
  CORE_MODES.forEach((m, i) => {
    ...
    d.className = 'mode';
    d.dataset.i = i;
    d.title = 'Core mode ' + m.name + ' — ' + m.desc +
              '  (key K cycles, ' +
              dez(CORE_SWITCH.time) + ' s to switch)';
...
function updateInspector() {
  ...
  el('insLevel').textContent = 'Level ' + b.level + '/' + UPGRADE.maxLevel;
  const rows = [['Structure', Math.ceil(b.hp) + '/' + b.maxHp]];
  if (b.def.turret) {
    const legt = !!b.def.minen;                  // der Minenleger zielt nicht
    rows.push([legt ? 'Damage per mine' : 'Damage', dez(game.stat(b, 'damage'))]);
    rows.push([legt ? 'Laying radius' : 'Range', dez(game.stat(b, 'range')) + ' tiles']);
    rows.push([legt ? 'Energy per mine' : 'Energy/shot', dez(game.energyOf(b))]);
    rows.push([legt ? 'Mine every' : 'Shot every', dez(game.cooldownOf(b), 2) + ' s']);
    if (b.def.arc) rows.push(['Jumps', b.def.arc + game.buffs.arcPlus]);
    if (legt) {
      let liegen = 0;
      for (const m of game.minen) if (m.owner === b) liegen++;
      rows.push(['Mines armed', liegen + '/' + game.minenZahl(b)]);
    } else rows.push(['Target priority', TARGETS[b.ziel || 0].name]);
    rows.push(['Load priority', PRIORITY[b.prio].name]);
    if (b.boost > 1) rows.push(['Amplifier Field', '+' + Math.round((b.boost - 1) * 100) + '%']);
  }
  if (b.def.turret && b.supplied && b.flow < 0.995)
    rows.push(['Grid throttle', '−' + Math.round((1 - b.flow) * 100) + '%']);
  // Werkdrohne und Schildfeld: was sie leisten und was sie dafür ziehen
  if (b.def.repair) {
    rows.push(['Repair', dez(game.repairRate(b)) + '/s']);
    rows.push(['Range', dez(game.stat(b, 'range')) + ' tiles']);
    rows.push(['Energy per structure', dez(b.def.perHp)]);
    rows.push(['Steady draw', dez(game.drawOf(b)) + '/s']);
    if (b.level >= UPGRADE.maxLevel)
      rows.push(['Emergency Weld', b.reserve ? 'ready' : 'used up']);
  }
  if (b.def.absorb) {
    rows.push(['Absorbs', Math.round(game.absorbOf(b) * 100) + '%']);
    rows.push(['Reserve', Math.round(b.puffer || 0) + '/' + game.schildPool(b)]);
    rows.push(['Range', dez(game.stat(b, 'range')) + ' tiles']);
    rows.push(['Charges', b.def.laden + '/s']);
    rows.push(['Energy per point', dez(b.def.perPoint)]);
    rows.push(['Draws', Math.round(b.def.ab * 100) + '% of buffer']);
    rows.push(['Field', !b.supplied ? 'unpowered'
                       : ((b.puffer || 0) > 0 ? 'charged' : 'empty')]);
  }
  if (b.def.regen) rows.push(['Yield', '+' + b.def.regen * b.level + '/s']);
  if (b.def.capacity) {
    rows.push(['Storage', '+' + Math.round(game.capOf(b))]);
    rows.push(['Carries', '+' + akkuFlow(b) + '/s']);
    if (b.level >= UPGRADE.maxLevel)
      rows.push(['Peak Load', b.reserve ? 'charged' : 'used up']);
  }
  if (b.def.supply)
    rows.push(['Grid radius', dez(game.netzRadius(b.def)) + ' tiles']);
  // Steht der Pylon auf einer alten Leiterbahn, sagt er es auch
  if (b.type === 'pylon' && b.leiter)
    rows.push(['Conduit', '+' + Math.round((GELAENDE.leiter - 1) * 100) + '% load']);
  // Am Pylon hängt die eigene Leitung, an allem anderen die des Knotens davor
  if (b.type === 'pylon' && b.supplied && b.node)
    rows.push(['Line load', dez(Math.max(0, b.node.through)) +
                               ' / ' + Math.round(b.node.cap) + '/s']);
  rows.push(['Power', b.def.needsPower ? (b.supplied ? 'connected' : 'CUT OFF') : '—']);
  el('insStats').innerHTML = rows.map(r => `<span>${r[0]}</span><span>${r[1]}</span>`).join('');
  const voll = b.level >= UPGRADE.maxLevel;
  el('upgradeBtn').textContent = voll ? 'Maxed' : 'Upgrade ' + game.upgradeCost(b);
  el('upgradeBtn').disabled = voll;
  el('sellBtn').textContent = 'Sell +' + Math.round(game.buildingValue(b) * game.buffs.refund);

  // Der Umzug-Knopf sagt im laufenden Zug, worauf er wartet
  const mv = el('moveBtn'), zieht = game.verschieben === b, mk = game.moveCost(b);
  mv.textContent = zieht ? 'Pick a tile — Esc cancels' : 'Move ' + mk;
  mv.classList.toggle('active', zieht);
  mv.disabled = !zieht && game.matter < mk;

  const rb = el('repairBtn');
  const heil = b.hp >= b.maxHp;
  rb.textContent = heil ? 'Intact' : 'Repair ' + game.repairCost(b);
  rb.disabled = heil;
  ...
  if (sp && b.level >= UPGRADE.maxLevel - 1) {
    ...
    spBox.innerHTML = '<b>' + (voll ? sp.name : 'Level 5: ' + sp.name) + '</b>' + sp.desc;
  ...
  if (b.def.turret) {
    ...
    el('zielBtn').textContent = 'Target';
    el('zielBtn').title = 'Target priority: ' + z.name + ' — ' + z.desc + '  (Z)';
    el('prioBtn').textContent = 'Load';
    el('prioBtn').title = 'Load priority: ' + PRIORITY[b.prio].name +
                          ' — who keeps firing when the buffer runs low  (L)';
    el('overloadBtn').textContent = b.overload ? 'Overload on' : 'Overload';
    el('overloadBtn').classList.toggle('on', !!b.overload);
    el('overloadBtn').title = 'Double damage, ' + game.buffs.overloadCost + '× energy use (O)';
  }
}
...
function bilanzHtml(b) {
  ...
  zeig(b.gegner, 'Enemies');
  zeig(b.energie, 'Energy fired');
  if (b.befehle) zeig(b.befehle, 'on core commands');
  zeig('+' + b.materie, 'Matter', 'gut');
  if (b.kernSchaden) zeig(b.kernSchaden, 'Core damage', 'schlecht');
  if (b.schild) zeig(b.schild, 'Energy as shield');
  if (b.verluste) zeig(b.verluste, b.verluste === 1 ? 'Building lost' : 'Buildings lost', 'schlecht');
  if (b.leer >= 0.3)
    zeig(dez(b.leer) + ' s', 'Buffer empty', 'schlecht');
  if (b.drossel > 0.02) zeig(Math.round(b.drossel * 100) + '%', 'Grid throttle', 'schlecht');
  if (b.bester)
    zeig(b.bester.schaden, 'Damage · ' + b.bester.name + ' Level ' + b.bester.level);
  // Wohin die nächste Welle gezogen wird — die einzige Zeile, die nach vorn schaut
  if (b.druck) zeig(b.druck, 'Pressure on the next wave', 'schlecht');
  return teile.join('');
}
...
function showDraft(cards) {
  ...
  cards.forEach((c, i) => {
    ...
    d.innerHTML = `<span class="num">${i + 1}</span><h3>${c.name}</h3><p>${c.desc}</p>` +
      (have ? `<span class="have">owned ${have}×</span>` : '');
...
function datumKurz(tag) {
  ...
  return t.length === 3 ? t[0] + '-' + t[1] + '-' + t[2] : String(tag || '');
}
...
function updateTagBadge() {
  ...
  if (!b.hidden) b.textContent = 'Daily map ' + datumKurz(game.tagesTag);
}
...
const TEIL_NAMEN = {
  ...
  akku:    ['Battery', 'Batteries'],
  blaster: ['Blaster', 'Blaster'],
  cannon:  ['Cannon', 'Cannons'],
  frost:   ['Frost Tower', 'Frost Towers'],
  wall:    ['Barrier', 'Barriers']
};
...
function bestenlisteHtml(liste, markiert) {
  ...
  const zeilen = liste.map((e, i) => {
    ...
    const wer = e === markiert
      ? '<input id="ovName" maxlength="' + NAME_MAX + '" placeholder="your name"' +
        ' aria-label="Name for the leaderboard" value="' + escHtml(e.name || '') + '">'
      : escHtml(e.name || '');
    return '<tr' + (e === markiert ? ' class="neu"' : '') + '>' +
           '<td class="platz">' + (i + 1) + '</td>' +
           (mitNamen ? '<td class="wer">' + wer + '</td>' : '') +
           '<td class="welle">Wave ' + e.wave + '</td>' +
           '<td class="wann">' + (e.tag ? 'Daily map ' + datumKurz(e.tag) : datum) + '</td>' +
...
function updateHud() {
  ...
  el('phaseLabel').textContent = build ? 'Build phase' : 'Attack';
  el('phaseValue').textContent = build
    ? Math.ceil(game.buildTimer) + 's'
    : (game.spawnQueue.length + game.enemies.length) + ' enemies';
  ...
  if (game.boss && !game.boss.dead) {
    ...
    st.textContent = b.guarded ? 'Shielded — take out the Wardens first'
                   : (b.enraged ? 'Enraged' : '');
  ...
  bw.hidden = !kommt;
  if (kommt)
    bw.innerHTML = 'Wave ' + (game.wave + 1) + ': <b>' + ENEMIES[kommt.type].name +
                   '</b> — ' + kommt.hint;
  ...
  if (game.alarm.on) {
    ...
    alarmEl.textContent = n > 1
      ? 'Core under fire — ' + n + ' enemies out of tower range'
      : 'Core under fire — no tower reaches there';
  }
  ...
  if (sturm)
    mb.innerHTML = (build ? 'Wave ' + (game.wave + 1) + ': ' : 'Storm wave: ') +
  ...
  if (game.overloadedNodes)
    nw.textContent = game.overloadedNodes === 1
      ? 'One line overloaded — towers behind it fire slower'
      : game.overloadedNodes + ' lines overloaded — towers behind them fire slower';
...
if (fortsetzen) {
  showOverlay('SAVED RUN FOUND',
    'You stopped after wave ' + game.wave + ' — ' + game.buildings.size +
    ' buildings, ' + Math.round(game.matter) + ' matter. The announced wave ' +
    (game.wave + 1) + ' is waiting unchanged.' +
    (game.tagesfeld() ? ' It is the daily map of ' + datumKurz(game.tagesTag) +
                        ' — it stays that way if you continue on another day.' : ''),
    bestenlisteHtml(game.bestenliste()));
  el('ovBtn').textContent = 'Continue';
  el('ovBtn').onclick = () => { game.gestartet = true; anzeigeSchliessen(); };
  el('ovBtn2').hidden = false;
  el('ovBtn2').textContent = 'Start over';
  el('ovBtn2').onclick = () => {
    standVerworfen = true;
    loesche(SAVE_KEY);
    location.reload();
  ...
  showOverlay('CORE DEFENSE',
    ...
    bestenlisteHtml(game.bestenliste()));
  el('ovBtn').textContent = 'Daily map ' + datumKurz(heutigerTag);
  el('ovBtn').onclick = () => { game.beginnen(heutigerTag); anzeigeSchliessen(); };
  el('ovBtn2').hidden = false;
  el('ovBtn2').textContent = 'Free map';
